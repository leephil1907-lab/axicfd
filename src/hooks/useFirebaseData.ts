...
import { onAuthStateChanged, User, signInWithPopup, GoogleAuthProvider, FacebookAuthProvider, signOut, signInWithEmailAndPassword, createUserWithEmailAndPassword, updateProfile } from 'firebase/auth';
...
export function useFirebaseData() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        const unsubUser = onSnapshot(userRef, async (docSnap) => {
          // Practice (demo) and live balances both start at 0 — no hardcoded seed funds
          let currLiveBal = 0;
          let currPracticeBal = 0;
          if (docSnap.exists()) {
            const data = docSnap.data();
            currPracticeBal = typeof data.balance === 'number' ? data.balance : 0;
            currLiveBal = typeof data.liveBalance === 'number' ? data.liveBalance : 0;
            setBalance(currPracticeBal);
            setLiveBalance(currLiveBal);
          } else {
            try {
              const isGoogle = currentUser.providerData.some(p => p.providerId === 'google.com');
              const displayName = isGoogle ? 'AXI' : (currentUser.displayName || (currentUser.email ? currentUser.email.split('@')[0] : 'Trader'));
              ...
              sendTelegramAlert('NEW_USER_REGISTRATION', 'New User Registered on AXITRADES', {
          ...
          // Mirror registration into admin-visible local directory (real user data only)
          try {
            ...
            const name = isGoogleUser ? 'AXI' : (currentUser.displayName || (email ? email.split('@')[0].toUpperCase() : 'Trader'));
            ...
            const updatedUserObj = {
              ...
              provider: currentUser.providerData[0]?.providerId || 'Email/Password',
              accountNo: existingIndex >= 0 && userList[existingIndex].accountNo ? userList[existingIndex].accountNo : `AXI-${currentUser.uid.substring(0, 8).toUpperCase()}`,
              accountType: existingIndex >= 0 && userList[existingIndex].accountType ? userList[existingIndex].accountType : 'Standard',
              leverage: existingIndex >= 0 && userList[existingIndex].leverage ? userList[existingIndex].leverage : '1:100',
              country: existingIndex >= 0 && userList[existingIndex].country ? userList[existingIndex].country : ''
            };
            ...
            if (existingIndex >= 0) {
              ...
              const welcomeEmailPayload = {
                ...
                recipientName: name,
                type: 'Registration',
                subject: `🎉 Welcome to AXITRADES - Account Registration Successful!`,
                timestamp: new Date().toUTCString(),
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      ...
      // Show "AXI" as the site display name after Google sign-in
      try {
        await updateProfile(result.user, { displayName: 'AXI' });
        await setDoc(
          doc(db, 'users', result.user.uid),
          { displayName: 'AXI', providerId: 'google.com', email: result.user.email || '' },
          { merge: true }
        );
      } catch (profileErr) {
        console.warn('Could not set display name to AXI:', profileErr);
      }
  ...
  const addTransaction = async (tx: any) => {
    ...
    sendTelegramAlert(
      ...
      `${emoji} ${isDeposit ? 'Deposit Request Initiated' : isWithdraw ? 'Withdrawal Payout Requested' : tx.type || 'Transaction'}: $${numAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD`,
      {
        ...
        'User Email': user?.email || tx.userEmail || 'guest',
        'User Name': user?.displayName || tx.user || (user?.email ? user.email.split('@')[0].toUpperCase() : 'Guest Trader'),
