...
export default function FundsView({ 
  ...
  const [cardProcessingMode, setCardProcessingMode] = useState<'direct' | 'stripe'>('stripe');
  ...
  const handleDeposit = async () => {
    ...
    if (val < 5) {
      showToast('Minimum deposit is $5 USD (Axi card/bank funding limit).', 'error');
      return;
    }

    const selectedObj = paymentMethods.find(m => m.id === selectedMethod);
    const methodName = selectedObj ? selectedObj.name : 'Deposit';
    const methodType = (selectedObj?.type || '').toLowerCase();
    const methodId = (selectedMethod || '').toLowerCase();

    // Card & bank transfer → Stripe Checkout (Axi-style secure funding)
    ...
    if (isStripeMethod) {
      ...
      try {
        const stripeMethod =
          methodType === 'bank' || methodId.includes('bank') || selectedMethod === 'manual_bank'
            ? 'bank'
            : 'card';

        const depositId = `DEP-${Math.random().toString(36).substr(2, 7).toUpperCase()}`;
        ...
        if (data.url) {
          // Record pending deposit before redirect (no balance credit)
          const newTx = {
            id: depositId,
            type: 'Deposit',
            amount: val,
            method: stripeMethod === 'bank' ? 'Bank Transfer (Stripe)' : 'Credit/Debit Card (Stripe)',
            date: new Date().toISOString().replace('T', ' ').substring(0, 19),
            status: 'Pending Verification',
            account: 'Live Account',
            refCode: data.id || depositId,
            proofNote: 'Redirected to Stripe Checkout — awaiting payment & admin approval',
          };
          ...
          showToast(
            stripeMethod === 'bank'
              ? 'Redirecting to secure Stripe bank transfer checkout...'
              : 'Redirecting to secure Stripe card checkout...',
            'info'
          );
        ...
        showToast(data.error || 'Stripe is not configured. Add STRIPE_SECRET_KEY to enable card/bank funding.', 'error');
      } catch (err: any) {
        console.error('Stripe checkout error:', err);
        showToast('Could not start Stripe checkout. Please try again or contact support.', 'error');
      } finally {
        setIsProcessing(false);
      }
      return;
    ...
    // Crypto / other methods — pending only (manual admin approval)
    const newTx = {
      ...
      proofNote: proofNote.trim() || 'Awaiting payment confirmation & admin approval',
    };
    if (addTransaction) {
    ...
    showToast(`DEPOSIT PENDING: $${val.toLocaleString()} via ${methodName} — pending admin verification.`, 'info');
    setStep('methods');
    setAmount('');
    setRefCode('');
    setProofNote('');
