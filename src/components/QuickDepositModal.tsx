  ...
  const handle3DSecureAuthorize = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const newTx = {
        ...
        type: 'Deposit',
        amount: numAmount,
        method: `${selectedMethod === 'card' ? 'Credit/Debit Card' : selectedMethod === 'applepay' ? 'Apple/Google Pay' : 'Bank Wire Direct'}`,
        date: new Date().toISOString().replace('T', ' ').substring(0, 19),
        status: 'Pending Verification',
        account: 'Live ECN Account',
        refCode: `DEP-${Math.floor(100000 + Math.random() * 899999)}`,
        proofNote: 'Awaiting manual admin approval — balance not credited until approved'
      };

      if (addTransaction) addTransaction(newTx);
      setTxReceipt(newTx);
      // No automatic balance credit — admin must approve
      setIsProcessing(false);
      setStep('receipt');
      if (showToast) showToast(`Deposit submitted for $${numAmount.toLocaleString()} USD. Pending admin verification.`, 'info');
    }, 1000);
  };
  ...
  const handlePaymentSuccess = (receiptInfo: any) => {
    if (addTransaction) {
      const newTx = {
        ...
        account: 'Live ECN Account',
        status: 'Pending Verification',
        proofNote: 'Payment recorded — awaiting manual admin approval before balance credit'
      };
      addTransaction(newTx);
      setTxReceipt(newTx);
    } else {
      setTxReceipt({ ...receiptInfo, status: 'Pending Verification' });
    }
    // No automatic balance credit — admin must approve
    setStep('receipt');
    if (showToast) showToast('Deposit recorded and pending admin verification. Balance will update after approval.', 'info');
  };
                ...
                <StripeCheckoutForm 
                   ...
                   onSuccess={(txResult) => {
                      const newTx = {
                        ...
                        proofNote: 'Stripe payment submitted — awaiting manual admin approval'
                      };
                      if (addTransaction) addTransaction(newTx);
                      setTxReceipt(newTx);
                      // No automatic balance credit — admin must approve
                      setStep('receipt');
                      if (showToast) showToast(`Deposit of $${Number(newTx.amount).toLocaleString()} USD submitted. Pending admin verification.`, 'info');
                   }}
