...
export default function HomeView({ quotes, setView, openSignUp }: HomeViewProps) {
  ...
  const selectedTrader = MASTER_TRADERS.find(t => t.id === selectedTraderId) || MASTER_TRADERS[0] || { id: '', name: '', avatar: '', roi: 0, winRate: 0, copiers: 0, riskScore: 0, profitGraph: [100] as number[], assetClass: '' };

  // Calculations for projected copy results (safe when no masters configured)
  const projectedMonthlyReturn = Math.round(allocation * ((selectedTrader.roi || 0) / 1200));
  const expectedCopierProfit = Math.round(allocation * ((selectedTrader.roi || 0) / 100));

  // Transform graph data for recharts
  const chartData = (selectedTrader.profitGraph || [100]).map((val: number, idx: number) => ({
    month: `Month ${idx + 1}`,
