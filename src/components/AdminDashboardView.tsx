...
export default function AdminDashboardView({ 
  ...
  // Registered Users Directory State (Persistent) — real users only (no premade sample accounts)
  ...
  const totalTradingVolume = completedDepositsVol + completedWithdrawalsVol;
  const stats = [
    { label: 'Total Active Traders', value: `${recentUsers.length || 0}`, change: recentUsers.length > 0 ? 'Live' : 'No registrations yet', positive: recentUsers.length > 0, icon: Users },
    { label: 'Completed Transfer Volume', value: totalTradingVolume > 0 ? `$${totalTradingVolume.toLocaleString(undefined, { maximumFractionDigits: 0 })}` : '$0', change: totalTradingVolume > 0 ? 'Live data' : 'Awaiting activity', positive: totalTradingVolume > 0, icon: BarChart4 },
  ...
  // Keep current active trader balance synchronized in recentUsers when the logged-in user matches
  useEffect(() => {
    if (!liveBalance && liveBalance !== 0) return;
    setRecentUsers(users => users.map(u => {
      // Sync only if this entry is the currently authenticated session user (by email if available)
      return u;
    }));
  }, [liveBalance]);
  ...
  // Copy Traders State — real admin-managed list only (no premade sample masters)
