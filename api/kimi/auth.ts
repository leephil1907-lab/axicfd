...
function getJwks() {
  if (!jwksInstance) {
    const urlString = env.kimiAuthUrl || "https://placeholder-kimi-auth.local";
    let url: URL;
    try {
      url = new URL(`${urlString}/api/.well-known/jwks.json`);
    } catch {
      url = new URL("https://placeholder-kimi-auth.local/api/.well-known/jwks.json");
    }
    jwksInstance = jose.createRemoteJWKSet(url);
  }
  return jwksInstance;
}
