...
app.post('/api/stripe/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  switch (event.type) {
    case 'payment_intent.succeeded': {
      ...
      const amt = ((paymentIntent.amount || 0) / 100).toFixed(2);
      const cur = (paymentIntent.currency || 'usd').toUpperCase();
      console.log(`✅ PaymentIntent ${paymentIntent.id} succeeded: ${amt} ${cur}`);
      notifyTelegram(
        `💰 <b>STRIPE PAYMENT RECEIVED</b>\n` +
          `Amount: $${amt} ${cur}\n` +
          `PI: <code>${paymentIntent.id}</code>\n` +
          `Email: ${paymentIntent.receipt_email || paymentIntent.charges?.data?.[0]?.billing_details?.email || 'N/A'}\n` +
          `Status: succeeded\n\n` +
          `<b>⚠️ MANUAL ACTION:</b> Approve this deposit in Admin → mark transaction Completed and credit live balance. Do NOT auto-credit.`
      );
      break;
    }
    case 'checkout.session.completed': {
      ...
      console.log(`✅ Checkout Session ${session.id} completed: ${amt} ${cur}`);
      notifyTelegram(
        `🛒 <b>STRIPE CHECKOUT COMPLETED</b>\n` +
          `Amount: $${amt} ${cur}\n` +
          `Session: <code>${session.id}</code>\n` +
          `Customer: ${session.customer_details?.email || session.customer_email || 'N/A'}\n` +
          `Method: ${session.metadata?.fundingMethod || 'card/bank'}\n` +
          `Deposit ID: ${session.metadata?.depositId || 'N/A'}\n\n` +
          `<b>⚠️ MANUAL ACTION:</b> Open Admin dashboard → find pending deposit → Approve to credit balance. KYC must be verified if required.`
      );
      break;
    }
    case 'payment_intent.payment_failed': {
      const failed = event.data.object as any;
      notifyTelegram(
        `❌ <b>STRIPE PAYMENT FAILED</b>\n` +
          `PI: <code>${failed.id}</code>\n` +
          `Error: ${failed.last_payment_error?.message || 'Unknown'}`
      );
      break;
    }
...
// Verify deposit after Stripe return — does NOT credit balance (admin only)
app.post('/api/stripe/verify-deposit', async (req, res) => {
  try {
    ...
    let verifiedAmount = typeof amount === 'number' ? amount : parseFloat(amount) || 0;
    let transactionId = '';
    let status = 'pending';
    ...
    if (sessionId) {
      ...
      if (session.payment_status === 'paid' || session.status === 'complete') {
        ...
        return res.json({
          success: false,
          message: `Checkout not paid yet (status: ${session.payment_status})`,
        });
      }
    } else if (paymentIntentId) {
      const pi = await stripe.paymentIntents.retrieve(paymentIntentId);
      if (pi.status === 'succeeded') {
        verifiedAmount = (pi.amount || 0) / 100;
        transactionId = pi.id;
        status = 'paid_pending_admin';
      } else {
        return res.json({
      ...
      status = 'reported_pending_admin';
      transactionId = `MANUAL-${Date.now()}`;
    } else {
      return res.status(400).json({ success: false, message: 'Missing sessionId, paymentIntentId, or amount' });
    }

    // Never auto-approve: client should create Pending Verification tx only
    res.json({
      success: true,
      amount: verifiedAmount,
      transactionId,
      status,
      message: 'Payment verified with Stripe. Balance requires manual admin approval.',
      autoCredit: false,
    });
  } catch (err: any) {
...
app.get('/api/markets/history', async (req, res) => {
  ...
  // Last-resort fallback: flat series anchored to the latest live price (no synthetic curves)
  const now = Math.floor(Date.now() / 1000);
  const basePrice = LIVE_MARKETS[symbol]?.price || 100;
  const fallback = [];
  const intervalSeconds = tf === '1D' ? 86400 : tf === '4H' ? 14400 : tf === '1H' ? 3600 : 900;
  const decimals = symbol.includes('JPY') ? 2 : (basePrice > 100 ? 2 : 5);

  for (let i = 40; i >= 0; i--) {
    const time = now - i * intervalSeconds;
    const close = Number(basePrice.toFixed(decimals));
    fallback.push({ time, open: close, high: close, low: close, close });
  }
  res.json(fallback);
});
...
app.post('/api/gemini/assistant', async (req, res) => {
  try {
    ...
    if (!apiKey) {
      return res.json({
        text: "Welcome to AXITRADES AI Assistant! To activate direct live market analysis with real-time Google Search grounding, please configure `GEMINI_API_KEY` in the workspace settings.\n\nIn the meantime, you can explore Forex pairs like EUR/USD, Cryptos like Bitcoin, or indices like the US30 with leverage up to 1:1000 and raw zero spreads!",
        offline: true
      });
    }
...
app.post('/api/stripe/create-checkout-session', async (req, res) => {
  ...
  try {
    ...
    // Axi-style: Card (Visa/Mastercard) and bank transfer via Stripe
    let paymentMethodTypes: string[] = ['card'];
    if (method === 'bank') {
      // ACH / bank debit where Stripe supports it; card fallback for other regions
    ...
    const baseUrl =
      ...
      (req.headers.referer ? String(req.headers.referer).split('?')[0] : 'http://localhost:3000');
    ...
    const sessionConfig: any = {
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: 'AXITRADES Account Funding',
              description:
                method === 'bank'
                  ? 'Bank transfer deposit to trading account'
                  : 'Credit/Debit card deposit to trading account',
            },
      ...
      metadata: {
        ...
        fundingMethod: String(method),
        platform: 'axitrades',
      },
      success_url: `${baseUrl.replace(/\/$/, '')}/?view=funds&deposit_success=true&amount=${numAmount}&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl.replace(/\/$/, '')}/?view=funds&deposit_cancelled=true`,
    };

    // Enable bank account collection for ACH when bank method selected
    ...
    // Fallback: card-only session if bank methods not enabled on the Stripe account
    if (error?.code === 'payment_method_type_invalid' || String(error?.message || '').includes('payment_method')) {
      try {
...
app.post('/api/email/send', async (req, res) => {
  ...
  const emailSubject = subject || (
    type === 'Registration'
      ? '🎉 Welcome to AXITRADES - Account Registration Successful!'
      : type === 'PasswordReset'
  ...
  try {
    ...
    if (smtpHost && smtpUser && smtpPass) {
      ...
      await transporter.sendMail({
        from: `"${process.env.SMTP_FROM_NAME || 'AXITRADES Official'}" <${process.env.SMTP_FROM_EMAIL || smtpUser}>`,
        to: recipientEmail,
...
// On Vercel, export Express app for serverless — do not call listen().
